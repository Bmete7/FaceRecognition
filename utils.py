...
def conv2d_bn(
  ...
  tensor = Conv2D(cv1_out, cv1_filter, strides=cv1_strides, name=layer+'_conv'+num,data_format="channels_last")(x)
  ...
  if cv2_out == None:
    return tensor
  tensor = Conv2D(cv2_out, cv2_filter, strides=cv2_strides, name=layer+'_conv'+'2',data_format="channels_last")(tensor)
